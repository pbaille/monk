(ns monk.vec
  (:refer-clojure :exclude [empty? type cat butlast last get drop take map rem assoc merge-with])
  (:require [clojure.core :as c]
            [monk.prelude :as u :refer [is]]))
...
(do :indexes
    ...
    ;; the second type of positions we have to deal with is those pointing between two elements
...
(defn put [v at x]
  (if-let [i (element-idx v at)]
    (c/assoc v i x)))

;; TODO it could be optimized for simple case
'(cond (contains? _ _) (c/update _ _ _))
(defn upd [v at f]
  (if-let [i (element-idx v at)]
    (c/update v i f)))
